...
const writableBytes = new Uint8Array(memory.buffer); // array of 8bit(1 byte) unsigned integers
...
writableBytes[i++] = (subroutineAddress & 0xff00) >> 8; //first two digits of 4 digit hex integer
writableBytes[i++] = subroutineAddress & 0x00ff; //last two digits of 4 digit hex integer
...
rl.on("line", () => {
  cpu.step();
  cpu.debug();
  cpu.viewMemoryAt(cpu.getRegister("ip"), 8);
  cpu.viewMemoryAt(0xffff - 1 - 42, 44);
});
